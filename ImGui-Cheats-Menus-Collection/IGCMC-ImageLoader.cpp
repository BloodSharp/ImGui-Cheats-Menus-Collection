...
bool IGCMC::Utils::LoadTextureFromFile(const char* filename, GLuint* out_texture, int* out_width, int* out_height)
{
    // Load from file
    int image_width = 0;
    int image_height = 0;
...
bool IGCMC::Utils::LoadTextureFromMemory(const unsigned char* buffer, int buffer_length, GLuint* out_texture, int* out_width, int* out_height)
{
    ...
    int image_width = 0;
    int image_height = 0;
    unsigned char* image_data = stbi_load_from_memory(buffer, buffer_length, &image_width, &image_height, NULL, 4);
    if (image_data == NULL)
        return false;
